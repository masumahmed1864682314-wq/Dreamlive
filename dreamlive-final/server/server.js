...
const CACHE_TTL = { live: 30_000, date: 60_000, upcoming: 15 * 60_000, detail: 5 * 60_000 };
const memoryCache = new Map();
const inflight = new Map();
const rate = new Map();

function bdDate(offset = 0) {
  const parts = new Intl.DateTimeFormat('en-CA', { timeZone: BD_TIMEZONE, year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(new Date());
...
function cacheFile(key) { return path.join(CACHE_DIR, `${key.replace(/[^a-z0-9_-]/gi, '_')}.json`); }
function readPersistent(key) { try { return JSON.parse(fs.readFileSync(cacheFile(key), 'utf8')); } catch { return null; } }
function writePersistent(key, value) { try { fs.writeFileSync(cacheFile(key), JSON.stringify(value)); } catch {} }
...
async function apiFootballDate(date) {
  ...
  const url = new URL(`${API_FOOTBALL_URL}/fixtures`);
  url.searchParams.set('date', date);
  url.searchParams.set('timezone', 'UTC');
  const r = await requestJSON(url, { headers: { 'x-apisports-key': API_FOOTBALL_KEY, accept: 'application/json' } });
  if (!r.ok || (r.data?.errors && Object.keys(r.data.errors).length)) throw new Error(JSON.stringify(r.data?.errors || r.data?.message || 'API-Football failed'));
  return Array.isArray(r.data?.response) ? r.data.response : [];
}
...
function normalizeFD(matches) {
  return matches.map(m => ({
    ...
    league: { id: m.competition?.id, name: m.competition?.name || 'Football', country: m.area?.name || '', logo: m.competition?.emblem || '' },
    teams: { home: { id: m.homeTeam?.id, name: m.homeTeam?.name || 'Home', logo: m.homeTeam?.crest || '' }, away: { id: m.awayTeam?.id, name: m.awayTeam?.name || 'Away', logo: m.awayTeam?.crest || '' } },
...
async function footballDataDate(date) {
  ...
  const url = new URL(`${FOOTBALL_DATA_URL}/matches`);
  url.searchParams.set('dateFrom', date); url.searchParams.set('dateTo', date);
  const r = await requestJSON(url, { headers: { 'X-Auth-Token': FOOTBALL_DATA_TOKEN, accept: 'application/json' } });
  if (!r.ok) throw new Error(r.data?.message || r.data?.error || 'football-data.org failed');
  return normalizeFD(r.data?.matches || []);
}

async function providerDate(date) {
  if (API_FOOTBALL_KEY) { try { return { provider: 'API-Football', matches: await apiFootballDate(date) }; } catch (e) { console.error('API-Football failed:', e.message); } }
  if (FOOTBALL_DATA_TOKEN) { try { return { provider: 'football-data.org', matches: await footballDataDate(date) }; } catch (e) { console.error('football-data.org failed:', e.message); } }
...
async function getDateData(date, force = false) {
  ...
  const p = (async () => {
    try {
      ...
      const payload = { success: true, source: out.provider, date, timezone: BD_TIMEZONE, response: out.matches };
...
app.get('/api/status', (_req,res) => res.json({ success:true, server:'DreamLive', timezone:BD_TIMEZONE, today:bdDate(), providers:{ apiFootball:Boolean(API_FOOTBALL_KEY), footballData:Boolean(FOOTBALL_DATA_TOKEN) }, cacheFiles: fs.readdirSync(CACHE_DIR).length }));
...
app.get('/api/live', async (req,res) => {
  try {
    ...
    res.json({ success:true, source:data.source, date, timezone:BD_TIMEZONE, results:matches.length, response:matches, cached:data.cached, stale:data.stale });
...
app.get('/api/upcoming', async (req,res) => {
  ...
  const promise = (async () => {
    try {
      ...
      for (let i=0;i<=2;i++) {
        const d = bdDate(i);
        const data = await getDateData(d, false);
      ...
      const payload = { success:true, source:source || 'fallback', from:key, to:bdDate(2), timezone:BD_TIMEZONE, results:matches.length, response:matches };
...
app.get('/api/match/:id', async (req,res) => {
  ...
  if (API_FOOTBALL_KEY) {
    try {
      const u = new URL(`${API_FOOTBALL_URL}/fixtures`); u.searchParams.set('id',id);
      const r = await requestJSON(u, { headers:{'x-apisports-key':API_FOOTBALL_KEY} });
      return res.status(r.status).json(r.data);
    } catch {}
  }
  if (FOOTBALL_DATA_TOKEN) {
    try { const r = await requestJSON(`${FOOTBALL_DATA_URL}/matches/${encodeURIComponent(id)}`, { headers:{'X-Auth-Token':FOOTBALL_DATA_TOKEN} }); return res.status(r.status).json(r.data); } catch {}
  }
  res.status(503).json({ success:false, error:'No match detail provider available.' });
});
...
app.post('/api/upload', (req,res,next) => {
  ...
  const supplied = req.get('x-admin-token') || req.body?.adminToken || '';
  if (supplied !== ADMIN_UPLOAD_TOKEN) return res.status(401).json({success:false,error:'Unauthorized'});
  next();
}, multer({ storage: multer.diskStorage({ destination:(req,file,cb)=>cb(null,UPLOADS_DIR), filename:(req,file,cb)=>{ const ext=path.extname(file.originalname); const base=path.basename(file.originalname,ext).replace(/[^a-zA-Z0-9-_]/g,'_').slice(0,80); cb(null,`${Date.now()}-${base}${ext}`); } }), limits:{fileSize:500*1024*1024}, fileFilter:(req,file,cb)=>cb(null,['video/mp4','video/webm','video/ogg','video/quicktime'].includes(file.mimetype)) }).single('video'), (req,res)=>{ if(!req.file) return res.status(400).json({success:false,error:'Please select a valid video.'}); res.json({success:true, video:`/uploads/${encodeURIComponent(req.file.filename)}`, filename:req.file.originalname}); });
...
app.use((err,_req,res,_next)=>res.status(400).json({success:false,error:err.message||'Something went wrong.'}));

app.listen(PORT,'0.0.0.0',()=>console.log(`DreamLive running on port ${PORT}`));
